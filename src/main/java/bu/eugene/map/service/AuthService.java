...
public class AuthService {
    ...
    public ResponseEntity registration(PersonDto dto) {
        ...
        emailService.sendSimpleMessage(person.getEmail(), "регистрация", "Спасибо за регистрацию");
    ...
    private void processRegistration(PersonDto dto) {
        Optional<Person> foundedPerson = personRepository.findByUsernameOrEmail(dto.getUsername(), dto.getEmail());
        if (foundedPerson.isPresent()) {
            throw new UsernameIsTakenException("юзернейм или почта занят другим пользователем!");
        }
    }
